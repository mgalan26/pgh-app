...
// ─── Widget de la pestaña "Nuevo evento" ──────────────────────────────────────

class AdminNuevoEventoTab extends ConsumerStatefulWidget {
  const AdminNuevoEventoTab({super.key});

  @override
  ConsumerState<AdminNuevoEventoTab> createState() =>
      _AdminNuevoEventoTabState();
}

class _AdminNuevoEventoTabState extends ConsumerState<AdminNuevoEventoTab> {
  ...
  void _resetForm() {
    ...
    _ciudadCtrl.clear();
    _urlOnlineCtrl.clear();
    _urlReservaCtrl.clear();
    _enlaceWebCtrl.clear();
    setState(() {
      _pais           = 'España';
      _tipo           = null;
      _entidadId      = null;
      ...
      _fechaInicio    = null;
      _horaInicio     = null;
      _tienePresencial = true;
      _tieneStreaming  = false;
      _esGratuito      = true;
    });
  }
  ...
  Future<void> _guardar() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _resetForm();
        // Volver a la pestaña Eventos (índice 0)
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            // ── Descripción ──────────────────────────────────────────────
            _tf(_descripcionCtrl, 'Descripción', maxLines: 5),
            const SizedBox(height: 12),
            ...
            if (_entidades == null)
              ...
              DropdownButtonFormField<String>(
                value: _entidadId,
                decoration:
                    const InputDecoration(labelText: 'Entidad organizadora *'),
                dropdownColor: AppTheme.darkCard,
                style: const TextStyle(color: AppTheme.textPrimary),
                isExpanded: true,
                items: [
                  const DropdownMenuItem(
                      value: null, child: Text('— Selecciona una entidad —')),
